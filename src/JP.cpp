...
void Pmue(std::vector<double>& a, std::vector<double>& L, std::vector<double>& E, std::vector<double>& P)
{
	...
	double A_CC, a0, a1, sqrt_a1, eigen[3], R_X[3], I_X[3], arcCos, L4E, Theta_10, Theta_20, Theta_21, denom;

    for (unsigned int n = 0; n < P.size(); ++n) {
        // Make  matter corrections (a0 and a1 have nothing to do with a)
        A_CC = a.at(n) / 3.0;
        a0 = a0_vac + 1.5 * (Y_ee * A_CC + H_ee * square(A_CC)) + cube(A_CC); // a0
        a1 = a1_vac + H_ee * A_CC + square(A_CC); // a1
        ...
        for (unsigned int i = 0; i < 3; ++i) {
            ...
            R_X[i] = ((eigen[i] + A_CC) * R_H_em + R_Y_em) / denom;
            I_X[i] = ((eigen[i] + A_CC) * I_H_em + I_Y_em) / denom;
        }

        // Compute Theta constants (not mixing angles)
        L4E = eVsqkm_to_GeV * L.at(n) / (4 * E.at(n));
        Theta_10 = (eigen[1] - eigen[0]) * L4E;
        Theta_20 = (eigen[2] - eigen[0]) * L4E;
        Theta_21 = (eigen[2] - eigen[1]) * L4E;

        // Compute probability
        P.at(n) = 2.0 * (- 2.0 * ((R_X[1]*R_X[0] + I_X[1]*I_X[0]) * square(sin(Theta_10))
